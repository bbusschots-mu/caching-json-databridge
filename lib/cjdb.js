 ...
 * callback will be mapped to the {@link Databridge.Datasource} object
 * the callback belongs to.
 * 
 * @callback DataFetcherCallback
 * @this Databridge.Datasource
...
const customValidators = {
    ...
    folderExists: function(val, opts){
        ...
        if(validate.isString(val) && fs.existsSync(val) && fs.statSync(val).isDirectory()){
...
class Databridge{
     ...
     * get.
     * @returns {Databridge.Datasource} a reference to the requested datasource
     ...
     * fetcher function.
     * @returns {Databridge.FetchResponse} a fetch response object containing
    ...
    fetchResponse(){
        ...
        let sourceName = args.datasourceName;
        ...
        let response = new FetchResponse(request, undefined, { cached: false });
        
        // before calling the fetcher, check for a valid cached copy
        //if(ds.option('enableCaching')){
        //    let cache = this._getStreamCache(sourceName, streamName);
        //    if(cache){
        ...
        //            response.meta('cached', cache.timestamp());
        //            
        ...
        //        try{
        //            self._writeStreamCache(new DataCache(
        //                sourceName,
        //                streamName,
        //                moment().toISOString(),
        //                data
        //            ));
        //        }catch(err){
        //            console.warn(`failed to cache data returned from datasource '${sourceName}' as stream '${streamName}'`, err);
        //        }
        //        return data;
        //    });
        //}
     ...
     */
    fetchDataPromise(datasourceName, options, fetcherArgs){
        return this.fetch(datasourceName, options, fetcherArgs).dataPromise();
    }
    ...
    _getStreamCache(sourceName, streamName){
        ...
        if(cachedObj){
            try{
                return DataCache.fromJsonObject(cachedObj);
            }catch(err){
    ...
    _writeStreamCache(cacheObj){
        var cachePath = this._generateCachePath(sourceName, streamName);
 ...
 *
 * @name Databridge.FetchRequest
 */
class FetchRequest{
    /**
     * @param {Databridge} bridge - a reference to the databridge the request
     * was submitted to.
     * @param {Databridge.Datasource} source - the datasource the request was
     * submitted on.
    ...
    constructor(){
        let args = validateParams.a(arguments, [
            ...
            },
            {
                vpopt_name: 'source',
                defined: true,
                isInstanceOf: [Databridge.Datasource]
            },
            {
...
Databridge.FetchRequest = FetchRequest

/**
 * An alias for `.databridge()`.
 * @returns {Databridge}
 * @see {Databridge.FetchRequest#databridge}
 */
 ...
 * An alias for `.datasource()`.
 * @returns {Databridge.Datasource}
 * @see {Databridge.FetchRequest#datasource}
 */
 ...
 * @returns {Object}
 * @see {Databridge.FetchRequest#fetchOptions}
 */
 ...
 * @returns {Array}
 * @see {Databridge.FetchRequest#fetcherParams}
 */
 ...
 *
 * @name Databridge.FetchResponse
 */
class FetchResponse{
    /**
     * @param {Databridge.FetchRequest} request - A reference to the data fetch
     ...
     * @returns {Databridge.FetchRequest}
     */
    request(){
...
Databridge.FetchResponse = FetchResponse
...
class DataCache{
    ...
    constructor(){
         ...
         * @type {DatabridgeName}
         */
        this._datasourceName = args.sourceName;
