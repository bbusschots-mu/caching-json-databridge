...
class Databridge{
     ...
     *
     * @param {DatasourceName} datasourceName - the name of the data source to
     * fetch the data with
     * @param {PlainObject} [options={}] - a plain object with options
     * @param {string} [options.datastreamName='main'] - for datasources that
    ...
    fetch(){
        let args = validateParams.assert(arguments, [
            validateParams.extendObject({ vpopt_name: 'datasourceName', presence: true }, vpCons.datasourceName),
            {
                paramOptions: {
                    name: 'options',
                    defaultWhenUndefined: {},
                    coerce: function(v, o, c){
                        if(validate.isObject(v)){
                            if(!validate.isDefined(v.datastreamName)) v.datastreamName = 'main';
                        }
                        return v;
                    }
                },
