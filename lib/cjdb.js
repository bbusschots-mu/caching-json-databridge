...
class Databridge{
    ...
    fetch(){
        ...
        if(ds.option('enableCaching')){
            let cache = this._getStreamCache(ds.name(), args.options.streamName);
            if(cache){
        ...
        // if no valid data was returned, try make a request to the data source
        let dataPromise = false;
        try{
            // LEFT OFF HERE
        }catch(err){
            throw new Error(`failed to fetch data from data source '${args.datasourceName}' with error: ${err.message}`);
        }
    }
