        ...
        if(ds.option('enableCaching')){
            ...
            if(cache){
                ...
                if(cache.isWithinTTL(ttl)){
                    // return the data in a pre-resolved promise
...
/**
 * An alias for `.fetchDataPromise()`.
 * @function
 * @see Databridge.fetchDataPromise
 */   
Databridge.prototype.fetch = Databridge.prototype.fetchDataPromise;
...
class FetchResponse{
    /**
     * @param {Databridge} bridge - A reference to the databridge that
     * performed the fetch.
     * 
     * @throws {external:validateParams.ValidationError} a validation error is
     * thrown if parameter validation fails.
