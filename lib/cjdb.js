...
class Databridge{
    ...
    fetchResponse(){
		...
		if(validate.isString(args.options.streamName)){
			...
			let streamNameGenerator = Databridge.defaultStreamNameGenerator;
...
/**
 * The default name generator function.
 *
 ...
 */
Databridge.defaultStreamNameGenerator = function(){
	let args = validateParams.assert(arguments, [
		{
			paramOptions: {
				name: 'fetcherParams',
	...
	if(args.fetcherParams.length === 1){
		return Databridge._valToNameString(args.fetcherParams[0]);
	}
	
	// if all else fails, try return an MD5 of the JSON string for the parameters - could throw an error
	return 'ph_' + stringToMD5Hex(args.fetcherParams);
};
 ...
 * @returns {string}
 */
Databridge._valToNameString = function(val){
	...
	switch(valType){
		...
		case 'boolean':
			return String(val);
		case 'number':
			preliminaryStr = 'n_' + val; // could contain decimal point, minus sign, etc.
			break;
		case 'string':
			...
			break;
		case 'object':
			return stringToMD5Hex(JSON.stringify(val));
		default:
			throw new Error(`can't convert type '${valType}' to string`);
	}
	if(validate.single(preliminaryStr, validateParams.extendObject({presence: true}, vpCons.databridgeName))){
		return preliminaryStr;
	}
	return stringToMD5Hex(preliminaryStr);
};
...
function stringToMD5Hex(str){
	return crypto.createHash('md5').update(JSON.stringify(str)).digest('hex');
}
